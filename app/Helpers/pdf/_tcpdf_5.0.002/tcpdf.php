 ...
 * TCPDF project (http://www.tcpdf.org) was originally derived in 2002 from the Public Domain FPDF class by Olivier
 * Plathey (http://www.fpdf.org), but now is almost entirely rewritten.<br>
 ...
 * <li>supports JPEG, PNG and SVG images natively, all images supported by GD (GD, GD2, GD2PART, GIF, JPEG, PNG, BMP,
 * XBM, XPM) and all images supported via ImagMagick (http://www.imagemagick.org/www/formats.html)</li>
 * <li>1D and 2D barcodes: CODE 39, ANSI MH10.8M-1983, USD-3, 3 of 9, CODE 93, USS-93, Standard 2 of 5, Interleaved 2
 * of 5, CODE 128 A/B/C, 2 and 5 Digits UPC-Based Extention, EAN 8, EAN 13, UPC-A, UPC-E, MSI, POSTNET, PLANET, RMS4CC
 * (Royal Mail 4-state Customer Code), CBC (Customer Bar Code), KIX (Klant index - Customer index), Intelligent Mail
 * Barcode, Onecode, USPS-B-3200, CODABAR, CODE 11, PHARMACODE, PHARMACODE TWO-TRACKS, QR-Code;</li>
 ...
 * @author Nicola Asuni
 * @copyright 2002-2010 Nicola Asuni - Tecnick.com S.r.l (www.tecnick.com) Via Della Pace, 11 - 09044 - Quartucciu (CA)
 *     - ITALY - www.tecnick.com - info@tecnick.com
	...
	class TCPDF
	{
		...
		public function Header()
		{
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
				$this->Image(K_PATH_IMAGES . $headerdata['logo'], $this->GetX(), $this->getHeaderMargin(),
		...
		public function Footer()
		{
			...
			if (!empty($barcode)) {
				...
				$this->write1DBarcode($barcode, 'C128B', $this->GetX(), $cur_y + $line_width, $barcode_width,
		...
		{
			return $this->GetArrStringWidth($this->utf8Bidi($this->UTF8StringToArray($s), $s, $this->tmprtl), $fontname,
				$fontstyle, $fontsize, $getarray);
		}
		...
		public function Annotation($x, $y, $w, $h, $text, $opt = ['Subtype' => 'Text'], $spaces = 0)
		{
			...
			if ((($opt['Subtype'] == 'FileAttachment') OR ($opt['Subtype'] == 'Sound')) AND (!$this->empty_string($opt['FS'])) AND file_exists($opt['FS']) AND (!isset($this->embeddedfiles[basename($opt['FS'])]))) {
				$this->embeddedfiles[basename($opt['FS'])] = [
					'file' => $opt['FS'],
		...
		protected function getCellCode(
			...
			if (is_string($border)) {
				...
				if (strpos($border, 'L') !== false) {
					...
					$s .= sprintf('%.2F %.2F m %.2F %.2F l S ', $xk, (($this->h - $y + $lm) * $k), $xk,
						(($this->h - ($y + $h + $lm)) * $k));
				}
				if (strpos($border, 'T') !== false) {
					...
					$s .= sprintf('%.2F %.2F m %.2F %.2F l S ', $xk, (($this->h - $y) * $k), $xwk,
						(($this->h - $y) * $k));
				}
				...
				if (strpos($border, 'B') !== false) {
					...
					$s .= sprintf('%.2F %.2F m %.2F %.2F l S ', $xk, (($this->h - ($y + $h)) * $k), $xwk,
						(($this->h - ($y + $h)) * $k));
				}
			}
			if ($txt != '') {
					...
					if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
						...
						if (defined('K_THAI_TOPCHARS') AND (K_THAI_TOPCHARS == true)) {
							...
							for ($i = 1; $i < $numchars; ++$i) {
								...
								if (in_array($unicode[$i], $topsym) AND (in_array($unicode[($i - 1)],
											$topsym) OR in_array($unicode[($i - 1)], $topchar))
								) {
				...
				if (($stretch > 0) AND (($ratio < 1) OR (($ratio > 1) AND (($stretch % 2) == 0)))) {
					if ($stretch > 2) {
						...
						$char_space = (($w - $width - (2 * $this->cMargin)) * $this->k) / max($this->GetNumChars($txt) - 1,
								1);
			...
			while ($i < $nb) {
				...
				if ($c == 10) { // 10 = "\n" = new line
					...
					if ((($this->CurrentFont['type'] == 'TrueTypeUnicode') OR ($this->CurrentFont['type'] == 'cidfont0')) AND ($arabic)) {
						...
						// *** very slow ***
						$l = $this->GetArrStringWidth($this->utf8Bidi(array_slice($chars, $j, ($i - $j)), '',
							$this->tmprtl));
					} else {
					...
					if (($l > $wmax) OR (($c == 173) AND (($l + $tmp_shy_replacement_width) > $wmax))) {
						...
						if ($sep == -1) {
							...
							$this->Cell($w, $h, $shy_char_left . $tmpstr . $shy_char_right, 0, 1, $align, $fill, $link,
								$stretch);
							unset($tmpstr);
							if ($firstline) {
		 ...
		 */
		public function getImageFileType($imgfile, $iminfo = [])
		{
		...
		public function Image(
			...
			if ($newimage) {
				...
				if ((method_exists($this, $mtd)) AND (!($resize AND function_exists($gdfunction)))) {
					...
					$info = $this->$mtd($file);
					if ($info == 'pngalpha') {
						return $this->ImagePngAlpha($file, $x, $y, $w, $h, 'PNG', $link, $align, $resize, $dpi,
							$palign);
					}
				}
				if (!$info) {
						...
						if ($type == 'SVG') {
							...
							if (preg_match('/<svg([^\>]*)>/si', $svgimg, $regs)) {
								...
								if (preg_match('/[\s]+width[\s]*=[\s]*"([^"]*)"/si', $regs[1], $tmp)) {
									...
									$svgimg = preg_replace('/[\s]+width[\s]*=[\s]*"[^"]*"/si',
										' width="' . $ow . $this->pdfunit . '"', $svgimg);
								}
								$tmp = [];
								if (preg_match('/[\s]+height[\s]*=[\s]*"([^"]*)"/si', $regs[1], $tmp)) {
									$oh = $this->getHTMLUnitToUnits($tmp[1], 1, $this->svgunit, false) * $dpi / 72;
									$svgimg = preg_replace('/[\s]+height[\s]*=[\s]*"[^"]*"/si',
										' height="' . $oh . $this->pdfunit . '"', $svgimg);
								}
								$tmp = [];
								if (!preg_match('/[\s]+viewBox[\s]*=[\s]*"[\s]*([0-9\.]+)[\s]+([0-9\.]+)[\s]+([0-9\.]+)[\s]+([0-9\.]+)[\s]*"/si',
									...
									$svgimg = preg_replace('/<svg/si', '<svg viewBox="0 0 ' . $vbw . ' ' . $vbh . '"',
										$svgimg);
								}
							}
			...
			$xkimg = $ximg * $this->k;
			$this->_out(sprintf('q %.2F 0 0 %.2F %.2F %.2F cm /I%d Do Q', ($w * $this->k), ($h * $this->k), $xkimg,
		 ...
		 * @see Image()
		 */
		protected function ImagePngAlpha(
			$file,
			$x = '',
			$y = '',
			$w = 0,
			...
			if ($this->sign) {
				...
				if (empty($this->signature_data['extracerts'])) {
					openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'],
						[$this->signature_data['privkey'], $this->signature_data['password']], [],
						PKCS7_BINARY | PKCS7_DETACHED);
				} else {
					openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'],
						[$this->signature_data['privkey'], $this->signature_data['password']], [],
						PKCS7_BINARY | PKCS7_DETACHED, $this->signature_data['extracerts']);
				}
				unlink($tempdoc);
				// read signature
				...
				$pdfdoc = substr($pdfdoc, 0, $byte_range[1]) . '<' . $signature . '>' . substr($pdfdoc,
						($byte_range[1]));
		...
		protected function _putpages()
		{
			...
			if (!empty($this->AliasNbPages)) {
				...
				if ($this->isunicode) {
					...
					$alias_cu = $this->_escape($this->utf8StrRev('{' . $this->AliasNbPages . '}', false,
						$this->tmprtl));
				}
			}
			if (!empty($this->AliasNumPage)) {
				...
				if ($this->isunicode) {
					...
					$alias_pcu = $this->_escape($this->utf8StrRev('{' . $this->AliasNumPage . '}', false,
						$this->tmprtl));
				}
			}
			$pagegroupnum = 0;
					...
					foreach ($this->PageAnnots[$n] as $key => $pl) {
						...
						switch (strtolower($pl['opt']['subtype'])) {
							case 'text': {
								...
								if (isset($pl['opt']['statemodel']) AND in_array($pl['opt']['statemodel'],
										$statemodels)
								) {
							...
							case 'link': {
								if (is_string($pl['txt'])) {
									...
									// internal link
									$l = $this->links[$pl['txt']];
									$annots .= sprintf(' /Dest [%d 0 R /XYZ 0 %.2F null]', (1 + (2 * $l[0])),
							...
							case 'widget': {
								...
								if (isset($pl['opt']['mk']) AND (is_array($pl['opt']['mk'])) AND !empty($pl['opt']['mk'])) {
									...
									if (isset($pl['opt']['mk']['if']) AND (is_array($pl['opt']['mk']['if'])) AND !empty($pl['opt']['mk']['if'])) {
										$annots .= ' /IF <<';
										$if_sw = ['A', 'B', 'S', 'N'];
										if (isset($pl['opt']['mk']['if']['sw']) AND in_array($pl['opt']['mk']['if']['sw'],
												$if_sw)
										) {
											$annots .= ' /SW /' . $pl['opt']['mk']['if']['sw'];
										}
										$if_s = ['A', 'P'];
										if (isset($pl['opt']['mk']['if']['s']) AND in_array($pl['opt']['mk']['if']['s'],
												$if_s)
										) {
											$annots .= ' /S /' . $pl['opt']['mk']['if']['s'];
										}
										if (isset($pl['opt']['mk']['if']['a']) AND (is_array($pl['opt']['mk']['if']['a'])) AND !empty($pl['opt']['mk']['if']['a'])) {
											$annots .= sprintf(' /A [%.2F  %.2F]', $pl['opt']['mk']['if']['a'][0],
												$pl['opt']['mk']['if']['a'][1]);
										}
		...
		protected function _putspotcolors()
		{
			foreach ($this->spot_colors as $name => $color) {
				...
				$out .= ' ' . sprintf('/C1 [%.4F %.4F %.4F %.4F] ', $color['c'] / 100, $color['m'] / 100,
		...
		protected function _enddoc()
		{
			...
			if ($this->sign AND isset($this->signature_data['cert_type'])) {
				...
				$signature_widget_ref .= str_repeat(' ',
					(strlen($this->sig_annot_ref) - strlen($signature_widget_ref)));
		...
		protected function _dounderlinew($x, $y, $w)
		{
			...
			return sprintf('%.2F %.2F %.2F %.2F re f', $x * $this->k, ($this->h - $y + ($linew / 2)) * $this->k,
				$w * $this->k, $linew);
		}
		...
		protected function _dolinethroughw($x, $y, $w)
		{
			...
			return sprintf('%.2F %.2F %.2F %.2F re f', $x * $this->k,
				($this->h - $y + ($this->FontSize / 3) + ($linew / 2)) * $this->k, $w * $this->k, $linew);
		}
		...
		protected function _dooverlinew($x, $y, $w)
		{
			...
			return sprintf('%.2F %.2F %.2F %.2F re f', $x * $this->k,
				($this->h - $y + $this->FontAscent - ($linew / 2)) * $this->k, $w * $this->k, $linew);
		...
		{
			$this->_out(sprintf('%.2F %.2F %.2F %.2F %.2F %.2F c', $x1 * $this->k, ($this->h - $y1) * $this->k,
				$x2 * $this->k, ($this->h - $y2) * $this->k, $x3 * $this->k, ($this->h - $y3) * $this->k));
		}
		...
		{
			$this->_out(sprintf('%.2F %.2F %.2F %.2F v', $x2 * $this->k, ($this->h - $y2) * $this->k, $x3 * $this->k,
				($this->h - $y3) * $this->k));
		}
		...
		{
			$this->_out(sprintf('%.2F %.2F %.2F %.2F y', $x1 * $this->k, ($this->h - $y1) * $this->k, $x3 * $this->k,
				($this->h - $y3) * $this->k));
		}
		 ...
		 * @since 4.9.019 (2010-04-26)
		 */
		protected function _outellipticalarc(
			$xc,
			$yc,
			$rx,
			$ry,
			...
			for ($i = 1; $i <= $nc; ++$i) {
				...
				// draw arc
				$this->_outCurve(($px1 + $qx1), ($this->h - ($py1 + $qy1)), ($px2 - $qx2), ($this->h - ($py2 - $qy2)),
					$px2, ($this->h - $py2));
				// move to next point
				$px1 = $px2;
				$py1 = $py2;
		...
		protected function _putbookmarks()
		{
			...
			foreach ($this->outlines as $i => $o) {
				...
				$out .= ' ' . sprintf('/Dest [%d 0 R /XYZ 0 %.2F null]', (1 + (2 * $o['p'])),
		...
		protected function _putjavascript()
		{
			...
			if (strpos($this->javascript, 'this.addField') > 0) {
				...
				$jsa = sprintf("ftcpdfdocsaved=this.addField('%s','%s',%d,[%.2F,%.2F,%.2F,%.2F]);", 'tcpdfdocsaved',
					'text', 0, 0, 1, 0, 1);
		...
		protected function _addfield($type, $name, $x, $y, $w, $h, $prop)
		{
			...
			$k = $this->k;
			$this->javascript .= sprintf("f" . $name . "=this.addField('%s','%s',%d,[%.2F,%.2F,%.2F,%.2F]);", $name,
					$type, $this->PageNo() - 1, $x * $k, ($this->h - $y) * $k + 1, ($x + $w) * $k,
					($this->h - $y - $h) * $k + 1) . "\n";
		...
		{
			$gs = $this->addExtGState(['ca' => $alpha, 'CA' => $alpha, 'BM' => '/' . $bm, 'AIS' => 'false']);
		 ...
		 * @access public
		 */
		public function cropMark($x, $y, $w, $h, $type = 'A,B,C,D', $color = [0, 0, 0])
		{
			$this->SetLineStyle([
				'width' => (0.5 / $this->k),
				'cap'   => 'butt',
		 ...
		 * @access public
		 */
		public function registrationMark($x, $y, $r, $double = false, $cola = [0, 0, 0], $colb = [255, 255, 255])
		{
			$line_style = [
				'width' => (0.5 / $this->k),
				'cap'   => 'butt',
			...
			if ($double) {
				...
				$this->Circle($x, $y, $r2, 0, 360, 'C', $line_style, [], 8);
			}
		}
		...
		{
			$this->Clip($x, $y, $w, $h);
			$this->Gradient(2, $coords, [
				['color' => $col1, 'offset' => 0, 'exponent' => 1],
		...
		protected function Clip($x, $y, $w, $h)
		{
			...
			$s = 'q';
			//set clipping area
			$s .= sprintf(' %.2F %.2F %.2F %.2F re W n', $x * $this->k, ($this->h - $y) * $this->k, $w * $this->k,
				-$h * $this->k);
			//set up transformation matrix for gradient
			$s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $w * $this->k, $h * $this->k, $x * $this->k,
				($this->h - ($y + $h)) * $this->k);
			$this->_out($s);
		}
		...
		public function Gradient($type, $coords, $stops, $background = [], $antialias = false)
		{
			...
			switch ($numcolspace) {
				case 4: { // CMYK
					...
					if (!empty($background)) {
						$this->gradients[$n]['background'] = sprintf('%.3F %.3F %.3F %.3F', $bcolor[0] / 100,
							$bcolor[1] / 100, $bcolor[2] / 100, $bcolor[3] / 100);
					}
					break;
				}
				case 3: { // RGB
					...
					if (!empty($background)) {
						$this->gradients[$n]['background'] = sprintf('%.3F %.3F %.3F', $bcolor[0] / 255,
							$bcolor[1] / 255, $bcolor[2] / 255);
					}
					break;
				}
				case 1: { // Gray scale
			...
			foreach ($stops as $key => $stop) {
				...
				switch ($numcolspace) {
					case 4: { // CMYK
						$this->gradients[$n]['colors'][$key]['color'] = sprintf('%.3F %.3F %.3F %.3F', $color[0] / 100,
							$color[1] / 100, $color[2] / 100, $color[3] / 100);
						break;
					}
					case 3: { // RGB
						$this->gradients[$n]['colors'][$key]['color'] = sprintf('%.3F %.3F %.3F', $color[0] / 255,
		...
		function _putshaders()
		{
			...
			foreach ($this->gradients as $id => $grad) {
				...
				if ($grad['type'] == 2) {
					$out .= ' ' . sprintf('/Coords [%.3F %.3F %.3F %.3F]', $grad['coords'][0], $grad['coords'][1],
					...
					$out .= ' ' . sprintf('/Coords [%.3F %.3F 0 %.3F %.3F %.3F]', $grad['coords'][0],
							$grad['coords'][1], $grad['coords'][2], $grad['coords'][3], $grad['coords'][4]);
					$out .= ' /Domain [0 1]';
					$out .= ' /Function ' . $fc . ' 0 R';
					$out .= ' /Extend [true true]';
					$out .= ' >>';
				...
				if ($grad['transparency']) {
					...
					$shading_transparency = preg_replace('/\/Function [0-9]+ /si', '/Function ' . $ft . ' ',
		...
		public function ImageEps(
			...
			if ($this->rasterize_vector_images) {
				...
				return $this->Image($file, $x, $y, $w, $h, 'EPS', $link, $align, true, 300, $palign, false, false,
			...
			// translate
			$this->_out(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F cm', 1, 0, 0, 1, $dx,
				$dy + ($this->hPt - (2 * $y * $k) - ($y2 - $y1))));
			// scale
			if (isset($scale_x)) {
		...
		public function write1DBarcode(
			...
			if ($arrcode !== false) {
				foreach ($arrcode['bcode'] as $k => $v) {
					...
					if ($v['t']) {
						...
						$this->Rect($xpos, $ypos, $bw, ($v['h'] * $barh / $arrcode['maxh']), 'F', [],
							$style['fgcolor']);
					}
					$xpos += $bw;
				}
			...
			if ($style['text']) {
				...
				$this->Cell(($arrcode['maxw'] * $xres), ($this->cell_height_ratio * $fontsize / $this->k), $code, 0, 0,
					'C', 0, '', $style['stretchtext']);
			}
		...
		protected function getHtmlDomArray($html)
		{
			...
			$html = strip_tags($html,
				'<marker/><a><b><blockquote><body><br><br/><dd><del><div><dl><dt><em><font><form><h1><h2><h3><h4><h5><h6><hr><i><img><input><label><li><ol><option><p><pre><select><small><span><strong><sub><sup><table><tablehead><tcpdf><td><textarea><th><thead><tr><tt><u><ul>');
			...
			while (($offset < strlen($html)) AND ($pos = strpos($html, '</option>', $offset)) !== false) {
				...
				while (preg_match("'<option([^\>]*)>(.*?)</option>'si", $html_b)) {
					$html_b = preg_replace("'<option([\s]+)value=\"([^\"]*)\"([^\>]*)>(.*?)</option>'si", "\\2\t\\4\r",
						$html_b);
						...
						if (isset($dom[$key]['attribute']['style'])) {
							...
							if (isset($dom[$key]['style']['font-size'])) {
								...
								switch ($fsize) {
									...
									default: {
										$dom[$key]['fontsize'] = $this->getHTMLUnitToUnits($fsize,
											$dom[$parentkey]['fontsize'], 'pt', true);
									}
								}
							}
							// line-height
			...
			while ($key < $maxel) {
				...
				if (($dom[$key]['value'] == 'tr') AND isset($dom[$key]['thead']) AND $dom[$key]['thead']) {
					if (isset($dom[$key]['parent']) AND isset($dom[$dom[$key]['parent']]['thead']) AND !$this->empty_string($dom[$dom[$key]['parent']]['thead'])) {
						...
						$this->writeHTML($this->thead, false, false, false, false, '');
						if (($this->start_transaction_page == ($this->numpages - 1)) OR ($this->y < $this->start_transaction_y) OR ($this->checkPageBreak($this->lasth,
								'', false))
						) {
							// restore previous object
						...
						if (($fontname != $curfontname) OR ($fontstyle != $curfontstyle) OR ($fontsize != $curfontsize)) {
							if (is_numeric($fontsize) AND ($fontsize >= 0)
								...
								if (!$dom[$key]['block']) {
									...
									$minstartliney = min($this->y, $minstartliney);
									$maxbottomliney = max(($this->y + (($fontsize * $this->cell_height_ratio) / $this->k)),
										$maxbottomliney);
								}
								$fontaligned = true;
							}
					...
					if (isset($startlinex)) {
						...
						if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl))))) OR ($yshift < 0)) {
							...
							if ($plalign == 'C') {
								...
								if (intval($this->revstrpos($pmid, ')]')) == (intval($this->revstrpos($pmid,
											' )]')) + 1)
								) {
								...
								if (($this->revstrpos($pmid, '[(') > 0) AND ((intval($this->revstrpos($pmid,
												'[( ')) == intval($this->revstrpos($pmid,
												'[('))) OR (intval($this->revstrpos($pmid,
												'[(' . chr(0) . chr(32))) == intval($this->revstrpos($pmid, '[('))))
								) {
									// remove first space (if any)
									$linew -= $one_space_width;
								}
								if ((strpos($pmid, '[(') > 0) AND (intval(strpos($pmid,
											'[(')) == (intval($this->revstrpos($pmid, '[('))))
								) {
									// remove last space (if any)
									$linew -= $one_space_width;
								...
								if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
									...
									while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece,
										...
										$epsposend = strpos($pmid, $this->epsmarker . 'Q',
												$offset) + strlen($this->epsmarker . 'Q');
										...
										switch ($strpiece[2][0]) {
											...
											case 'l': {
												...
												preg_match('/([0-9\.\+\-]*)[\s](' . $strpiece[1][0] . ')[\s](' . $strpiece[2][0] . ')([\s]*)/x',
													$pmid, $xmatches);
											...
											case 're': {
												// justify block
												preg_match('/([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s](' . $strpiece[1][0] . ')[\s](re)([\s]*)/x',
													$pmid, $xmatches);
												$currentxpos = $xmatches[1];
											...
											case 'c': {
												// get current X position
												preg_match('/([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s]([0-9\.\+\-]*)[\s](' . $strpiece[1][0] . ')[\s](c)([\s]*)/x',
													$pmid, $xmatches);
												$currentxpos = $xmatches[1];
												// justify block
									...
									if (($this->CurrentFont['type'] == 'TrueTypeUnicode') OR ($this->CurrentFont['type'] == 'cidfont0')) {
										...
										$this->setPageBuffer($startlinepage,
											$pstart . "\n" . $pmid . "\nBT 0 Tw ET\n" . $pend);
										$endlinepos = strlen($pstart . "\n" . $pmid . "\nBT 0 Tw ET\n");
									}
								}
				...
				if ($dom[$key]['tag']) {
					if ($dom[$key]['opening']) {
						...
						if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
							...
							if (isset($dom[($dom[$trid]['parent'])]['attribute']['cellpadding'])) {
								$currentcmargin = $this->getHTMLUnitToUnits($dom[($dom[$trid]['parent'])]['attribute']['cellpadding'],
									1, 'px');
							} else {
								$currentcmargin = 0;
							}
							...
							if (isset($dom[($dom[$trid]['parent'])]['attribute']['cellspacing'])) {
								$cellspacing = $this->getHTMLUnitToUnits($dom[($dom[$trid]['parent'])]['attribute']['cellspacing'],
									1, 'px');
							} else {
								$cellspacing = 0;
							}
								...
								while ($rsk < $rskmax) {
									...
									if (($trwsp['rowspan'] > 0)
										...
										if (($trwsp['rowspan'] == 1)
											...
											$dom[$table_el]['rowspans'][$rsk]['endy'] = max($dom[$trid]['endy'],
												$trwsp['endy']);
							...
							$this->MultiCell($cellw, $cellh, $cell_content, false, $lalign, false, 2, '', '', true, 0,
								true);
									...
									if ($trwsp['rowspan'] > 0) {
										if (isset($dom[$trid]['endpage'])) {
											if ($trwsp['endpage'] == $dom[$trid]['endpage']) {
												$dom[$table_el]['rowspans'][$k]['endy'] = max($dom[$trid]['endy'],
													$trwsp['endy']);
					...
					if (!$this->empty_string($this->lispacer)) {
						...
						if (is_numeric($pfontsize) AND ($pfontsize > 0) AND is_numeric($curfontsize) AND ($curfontsize > 0) AND ($pfontsize != $curfontsize)) {
							...
							$minstartliney = min($this->y, $minstartliney);
							$maxbottomliney = max(($this->y + (($pfontsize * $this->cell_height_ratio) / $this->k)),
								$maxbottomliney);
						}
					}
					// text
					...
					if (!empty($this->HREF) AND (isset($this->HREF['url']))) {
						// HTML <a> Link
						$strrest = $this->addHtmlLink($this->HREF['url'], $dom[$key]['value'], $wfill, true,
							$this->HREF['color'], $this->HREF['style'], true);
					} else {
			...
			switch ($tag['value']) {
				...
				case 'a': {
					...
					if (array_key_exists('style', $tag['attribute'])) {
						// get style attributes
						preg_match_all('/([^;:\s]*):([^;]*)/', $tag['attribute']['style'], $style_array,
							PREG_PATTERN_ORDER);
						$astyle = [];
						while (list($id, $name) = each($style_array[1])) {
				...
				case 'img': {
					if (isset($tag['attribute']['src'])) {
						...
						if (($type == 'eps') OR ($type == 'ai')) {
							...
							$this->Image($tag['attribute']['src'], $xpos, $this->y, $iw, $ih, '', $imglink, $align,
								false, 300, '', false, false, $border, false, false, true);
						}
						switch ($align) {
							case 'T': {
								$this->y = $prevy;
			...
			switch ($tag['value']) {
				case 'tr': {
					...
					if (isset($dom[$table_el]['rowspans'])) {
						foreach ($dom[$table_el]['rowspans'] as $k => $trwsp) {
							...
							if ($dom[$table_el]['rowspans'][$k]['rowspan'] == 0) {
								if ($dom[$table_el]['rowspans'][$k]['endpage'] == $parent['endpage']) {
									$dom[($dom[$key]['parent'])]['endy'] = max($dom[$table_el]['rowspans'][$k]['endy'],
										$parent['endy']);
						...
						foreach ($dom[$table_el]['rowspans'] as $k => $trwsp) {
							if ($dom[$table_el]['rowspans'][$k]['rowspan'] == 0) {
								$dom[$table_el]['rowspans'][$k]['endpage'] = max($dom[$table_el]['rowspans'][$k]['endpage'],
									$dom[($dom[$key]['parent'])]['endpage']);
								$dom[($dom[$key]['parent'])]['endpage'] = $dom[$table_el]['rowspans'][$k]['endpage'];
								$dom[$table_el]['rowspans'][$k]['endy'] = max($dom[$table_el]['rowspans'][$k]['endy'],
					...
					if (($this->num_columns > 1) AND ($dom[($dom[$key]['parent'])]['endy'] >= ($this->PageBreakTrigger - $this->lasth)) AND ($this->y < $dom[($dom[$key]['parent'])]['endy'])) {
						...
						if (isset($dom[$table_el]['attribute']['cellspacing'])) {
							$cellspacing = $this->getHTMLUnitToUnits($dom[$table_el]['attribute']['cellspacing'], 1,
								'px');
							$this->y += $cellspacing;
						}
						$this->Ln(0, $cell);
						...
						foreach ($parent['cellpos'] as $k => $cellpos) {
							...
							if ($endpage > $startpage) {
								...
								for ($page = $startpage; $page <= $endpage; ++$page) {
									...
									// design a cell around the text
									$ccode = $this->FillColor . "\n" . $this->getCellCode($cw, $ch, '', $cborder, 1, '',
											$fill, '', 0, true);
									if ($cborder OR $fill) {
										$pagebuff = $this->getPageBuffer($this->page);
										$pstart = substr($pagebuff, 0, $this->intmrk[$this->page]);
								...
								// design a cell around the text
								$ccode = $this->FillColor . "\n" . $this->getCellCode($cw, $ch, '', $border, 1, '',
										$fill, '', 0, true);
								if ($border OR $fill) {
									if (end($this->transfmrk[$this->page]) !== false) {
										$pagemarkkey = key($this->transfmrk[$this->page]);
		...
		protected function putHtmlListBullet($listdepth, $listtype = '', $size = 10)
		{
			...
			switch ($listtype) {
				...
				case 'circle': {
					...
					$this->Circle(($this->x + $r), ($this->y + ($this->lasth / 2)), $r, 0, 360, $fill,
						['color' => $color], $color, 8);
					break;
				}
				case 'square': {
		 ...
		 */
		public function addHTMLTOC($page = '', $toc_name = 'TOC', $templates = [], $correct_align = true)
		{
		 ...
		 * @access protected
		 */
		protected function hyphenateWord(
			$word,
			$patterns,
			$dictionary = [],
			$leftmin = 1,
			...
			$tmpword = array_merge([95], $word, [95]);
		...
		public function hyphenateText(
			...
			foreach ($unichars as $char) {
				if ((!$intag) AND $unicode[$char] == 'L') {
					...
					if (!$this->empty_string($word)) {
						// hypenate the word
						$txtarr = array_merge($txtarr,
							$this->hyphenateWord($word, $patterns, $dictionary, $leftmin, $rightmin, $charmin,
								$charmax));
						$word = [];
					}
					$txtarr[] = $char;
			...
			if (!$this->empty_string($word)) {
				// hypenate the word
				$txtarr = array_merge($txtarr,
					$this->hyphenateWord($word, $patterns, $dictionary, $leftmin, $rightmin, $charmin, $charmax));
			}
		...
		public function ImageSVG(
			...
			if ($this->rasterize_vector_images) {
				// convert SVG to raster image using GD or ImageMagick libraries
				return $this->Image($file, $x, $y, $w, $h, 'SVG', $link, $align, true, 300, $palign, false, false,
			...
			$this->_out(sprintf('%.3F %.3F %.3F %.3F %.3F %.3F cm', $svgscale_x, 0, 0, $svgscale_y, $e + $svgoffset_x,
				$f + $svgoffset_y));
			...
			if (!xml_parse($this->parser, $svgdata)) {
				$error_message = sprintf("SVG Error: %s at line %d",
		...
		protected function getSVGTransformMatrix($attribute)
		{
			...
			while ($continue) {
				...
				// matrix
				$regs = [];
				if (preg_match('/matrix\(([a-z0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)\)/si',
					$attribute, $regs)) {
					...
					$e = $regs[5];
					$f = $regs[6];
					$tm = $this->getTransformationMatrixProduct($tm, [$a, $b, $c, $d, $e, $f]);
				}
				// translate x
				$regs = [];
				...
				if (preg_match('/translate\(([a-z0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)\)/si', $attribute, $regs)) {
					...
					$e = $regs[1];
					$f = $regs[2];
					$tm = $this->getTransformationMatrixProduct($tm, [1, 0, 0, 1, $e, $f]);
				}
				// scale x
				$regs = [];
				...
				if (preg_match('/scale\(([a-z0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)\)/si', $attribute, $regs)) {
					...
					$tm = $this->getTransformationMatrixProduct($tm, [$a, 0, 0, $d, 0, 0]);
				}
				// rotate ang
				$regs = [];
				...
				// rotate ang,x,y
				$regs = [];
				if (preg_match('/rotate\(([0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)[\,\s]+([a-z0-9\-\.]+)\)/si', $attribute,
					$regs)) {
					$attribute = str_replace($regs[0], '', $attribute);
					$continue = true;
					$ang = deg2rad($regs[1]);
				...
				if (preg_match('/skewX\(([0-9\-\.]+)\)/si', $attribute, $regs)) {
					...
					$tm = $this->getTransformationMatrixProduct($tm, [1, 0, $c, 1, 0, 0]);
				}
				// skewY
				$regs = [];
				if (preg_match('/skewY\(([0-9\-\.]+)\)/si', $attribute, $regs)) {
					...
					$tm = $this->getTransformationMatrixProduct($tm, [1, $b, 0, 1, 0, 0]);
				}
			}
		 ...
		 * @access protected
		 */
		protected function setSVGStyles(
			$svgstyle,
			$prevsvgstyle,
			$x = 0,
			$y = 0,
			...
			if (preg_match('/url\([\s]*\#([^\)]*)\)/si', $svgstyle['fill'], $regs)) {
				...
				if ($gradient['mode'] == 'measure') {
					if (isset($gradient['gradientTransform']) AND !empty($gradient['gradientTransform'])) {
						...
						if (isset($gradient['coords'][4])) {
							$gradient['coords'][4] = sqrt(pow(($gtm[0] * $gradient['coords'][4]),
									2) + pow(($gtm[1] * $gradient['coords'][4]), 2));
						}
					...
					$gradient['coords'][0] = $this->getHTMLUnitToUnits($gradient['coords'][0], 0, $this->svgunit,
						false);
					$gradient['coords'][1] = $this->getHTMLUnitToUnits($gradient['coords'][1], 0, $this->svgunit,
						false);
					$gradient['coords'][2] = $this->getHTMLUnitToUnits($gradient['coords'][2], 0, $this->svgunit,
						false);
					$gradient['coords'][3] = $this->getHTMLUnitToUnits($gradient['coords'][3], 0, $this->svgunit,
						false);
					if (isset($gradient['coords'][4])) {
						$gradient['coords'][4] = $this->getHTMLUnitToUnits($gradient['coords'][4], 0, $this->svgunit,
							false);
					}
					// shift units
				...
				if (($gradient['type'] == 3) AND ($gradient['mode'] == 'measure')) {
					...
					$this->_out(sprintf('%.3F 0 0 %.3F %.3F %.3F cm', $w * $this->k, $w * $this->k, $x * $this->k,
						$cy * $this->k));
				} else {
					$this->_out(sprintf('%.3F 0 0 %.3F %.3F %.3F cm', $w * $this->k, $h * $this->k, $x * $this->k,
						($this->h - ($y + $h)) * $this->k));
				}
			...
			$font_size = $this->getHTMLUnitToUnits($font_size, $prevsvgstyle['font-size'], $this->svgunit,
					false) * $this->k;
			switch ($font_style) {
				case 'italic': {
					$font_style = 'I';
		...
		protected function startSVGElementHandler($parser, $name, $attribs)
		{
			...
			if ($this->svgclipmode) {
				$this->svgclippaths[$this->svgclipid][] = ['name' => $name, 'attribs' => $attribs];
			...
			foreach ($prev_svgstyle as $key => $val) {
				if (isset($attribs[$key])) {
					...
					// CSS style syntax
					$attrval = [];
					if (preg_match('/[;\"\s]{1}' . $key . '[\s]*:[\s]*([^;\"\s]*)/si', $attribs['style'],
							$attrval) AND isset($attrval[1])
					) {
			...
			switch ($name) {
				...
				case 'path': {
					...
					if ($clipping) {
						...
						$this->StartTransform();
						$this->SVGTransform($tm);
						$obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, 0, 0, 1, 1, 'SVGPath', [$d, 'CNZ']);
						if (!empty($obstyle)) {
				...
				// shapes
				case 'rect': {
					$x = (isset($attribs['x']) ? $this->getHTMLUnitToUnits($attribs['x'], 0, $this->svgunit,
						false) : 0);
					$y = (isset($attribs['y']) ? $this->getHTMLUnitToUnits($attribs['y'], 0, $this->svgunit,
						false) : 0);
					$w = (isset($attribs['width']) ? $this->getHTMLUnitToUnits($attribs['width'], 0, $this->svgunit,
						false) : 0);
					$h = (isset($attribs['height']) ? $this->getHTMLUnitToUnits($attribs['height'], 0, $this->svgunit,
						false) : 0);
					$rx = (isset($attribs['rx']) ? $this->getHTMLUnitToUnits($attribs['rx'], 0, $this->svgunit,
						false) : 0);
					$ry = (isset($attribs['ry']) ? $this->getHTMLUnitToUnits($attribs['ry'], 0, $this->svgunit,
						false) : $rx);
					if ($clipping) {
						$this->SVGTransform($tm);
						...
						$this->StartTransform();
						$this->SVGTransform($tm);
						$obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, $x, $y, $w, $h, 'RoundedRectXY',
							[$x, $y, $w, $h, $rx, $ry, '1111', 'CNZ']);
						if (!empty($obstyle)) {
				...
				case 'circle': {
					$cx = (isset($attribs['cx']) ? $this->getHTMLUnitToUnits($attribs['cx'], 0, $this->svgunit,
						false) : 0);
					$cy = (isset($attribs['cy']) ? $this->getHTMLUnitToUnits($attribs['cy'], 0, $this->svgunit,
						false) : 0);
					$r = (isset($attribs['r']) ? $this->getHTMLUnitToUnits($attribs['r'], 0, $this->svgunit,
						false) : 0);
					$x = $cx - $r;
					$y = $cy - $r;
					$w = 2 * $r;
					...
					if ($clipping) {
						...
						$this->StartTransform();
						$this->SVGTransform($tm);
						$obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, $x, $y, $w, $h, 'Circle',
							[$cx, $cy, $r, 0, 360, 'CNZ']);
						if (!empty($obstyle)) {
				...
				case 'ellipse': {
					...
					if ($clipping) {
						...
						$this->StartTransform();
						$this->SVGTransform($tm);
						$obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, $x, $y, $w, $h, 'Ellipse',
							[$cx, $cy, $rx, $ry, 0, 0, 360, 'CNZ']);
						if (!empty($obstyle)) {
				...
				case 'line': {
					$x1 = (isset($attribs['x1']) ? $this->getHTMLUnitToUnits($attribs['x1'], 0, $this->svgunit,
						false) : 0);
					$y1 = (isset($attribs['y1']) ? $this->getHTMLUnitToUnits($attribs['y1'], 0, $this->svgunit,
						false) : 0);
					$x2 = (isset($attribs['x2']) ? $this->getHTMLUnitToUnits($attribs['x2'], 0, $this->svgunit,
						false) : 0);
					$y2 = (isset($attribs['y2']) ? $this->getHTMLUnitToUnits($attribs['y2'], 0, $this->svgunit,
						false) : 0);
					$x = $x1;
					$y = $y1;
					$w = abs($x2 - $x1);
					...
					if (!$clipping) {
						$this->StartTransform();
						$this->SVGTransform($tm);
						$obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, $x, $y, $w, $h, 'Line',
							[$x1, $y1, $x2, $y2]);
						$this->Line($x1, $y1, $x2, $y2);
						$this->StopTransform();
					}
				...
				case 'polygon': {
					...
					if ($name == 'polyline') {
						$this->StartTransform();
						$this->SVGTransform($tm);
						$obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, $x, $y, $w, $h, 'PolyLine',
							[$p, 'CNZ']);
						$this->PolyLine($p, 'D', [], []);
						$this->StopTransform();
					} else { // polygon
						if ($clipping) {
							...
							$this->StartTransform();
							$this->SVGTransform($tm);
							$obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, $x, $y, $w, $h, 'Polygon',
								[$p, 'CNZ']);
							if (!empty($obstyle)) {
