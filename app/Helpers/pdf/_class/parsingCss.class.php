...
{
public $value = [];
public $css = []; // list of the HTML colors
    public $cssKeys = [];   // flag if we are in a sub html => only "text-align:left" is used
    public $table = [];    // default font to use if the asked font does not exist
    /**
     * reference to the pdf object
     * @var TCPDF
     */
    protected $_pdf = null; // current values
    protected $_htmlColor = []; // css values
    ...
    public function initStyle()
    {
        ...
        $this->value['id_lst'] = ['*'];   // tag - list of legacy
        ...
        $this->value['color'] = [0, 0, 0];
        $this->value['background'] = ['color' => null, 'image' => null, 'position' => null, 'repeat' => null];
        $this->value['border'] = [];
        $this->value['padding'] = [];
        $this->value['margin'] = [];
    ...
    public function resetStyle($tagName = '')
    {
        ...
        if (!in_array($tagName, ['tr', 'td', 'th', 'thead', 'tbody', 'tfoot'])) {
            $collapse = false;
        }
        ...
        if (!in_array($tagName, ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])) {
            $this->value['margin'] = ['t' => 0, 'r' => 0, 'b' => 0, 'l' => 0];
        }
        ...
        if (in_array($tagName, ['blockquote', 'div', 'fieldset'])) {
            $this->value['vertical-align'] = 'top';
        }

        if (in_array($tagName, ['fieldset', 'legend'])) {
            $this->value['border'] = [
                't'        => $border,
                'r'        => $border,
                'b'        => $border,
                'l'        => $border,
                'radius'   => [
                    'tl' => [$units['5px'], $units['5px']],
                    'tr' => [$units['5px'], $units['5px']],
                    'br' => [$units['5px'], $units['5px']],
                    'bl' => [$units['5px'], $units['5px']]
                ],
                'collapse' => false,
            ];
        }

        if (in_array($tagName, ['ul', 'li'])) {
            $this->value['list-style-type'] = '';
            $this->value['list-style-image'] = '';
        }

        if (!in_array($tagName, ['tr', 'td'])) {
            $this->value['padding'] = [
                't' => 0,
                'r' => 0,
                'b' => 0,
    ...
    {
        // border none
        $none = ['type' => 'none', 'width' => 0, 'color' => [0, 0, 0]];
        ...
        foreach ($css as $value) {
            ...
            if ($tmp !== null) {
                ...
                if (in_array($value, ['solid', 'dotted', 'dashed', 'double'])) {
                    $type = $value;
        ...
        return ['type' => $type, 'width' => $width, 'color' => $color];
    }
    ...
    public function convertToColor($css, &$res)
    {
        ...
        if (preg_match('/cmyk\([\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*,[\s]*([0-9%\.]+)[\s]*\)/isU',
            $css, $match)) {
    ...
    public function analyse($tagName, &$param, $legacy = null)
    {
        ...
        foreach ($styles as $nom => $val) {
            switch ($nom) {
                ...
                case 'text-transform':
                    if (!in_array($val, ['none', 'capitalize', 'uppercase', 'lowercase'])) {
                        $val = 'none';
                    }
                ...
                case 'text-align':
                    $val = strtolower($val);
                    if (!in_array($val, ['left', 'right', 'center', 'justify', 'li_right'])) {
                        $val = 'left';
                    }
                ...
                case 'rotate':
                    if (!in_array($val, [0, -90, 90, 180, 270, -180, -270])) {
                        $val = null;
                    }
                    if ($val < 0) {
                        $val += 360;
                ...
                case 'overflow':
                    if (!in_array($val, ['visible', 'hidden'])) {
                        $val = 'visible';
                    }
                ...
                case 'border-style':
                    ...
                    foreach ($val as $valK => $valV) {
                        if (!in_array($valV, ['solid', 'dotted', 'dashed'])) {
                            $val[$valK] = null;
                        }
                    }
                    $this->_duplicateBorder($val);
                ...
                case 'border-top-style':
                    if (in_array($val, ['solid', 'dotted', 'dashed'])) {
                ...
                case 'border-radius':
                    ...
                    if (!isset($valH[2])) {
                        $valH = [$valH[0], $valH[0], $valH[1], $valH[1]];
                    }
                    if (!isset($valH[3])) {
                        $valH[3] = $valH[1];
                    }
                    if (isset($val[1])) {
                        ...
                        if (!isset($valV[2])) {
                            $valV = [$valV[0], $valV[0], $valV[1], $valV[1]];
                        }
                        if (!isset($valV[3])) {
                            $valV[3] = $valV[1];
                        }
                ...
                case 'border-top-left-radius':
                    ...
                    $this->value['border']['radius']['tl'] = [$val[0], isset($val[1]) ? $val[1] : $val[0]];
                    break;
                ...
                case 'border-top-right-radius':
                    ...
                    $this->value['border']['radius']['tr'] = [$val[0], isset($val[1]) ? $val[1] : $val[0]];
                    break;
                ...
                case 'border-bottom-right-radius':
                    ...
                    $this->value['border']['radius']['br'] = [$val[0], isset($val[1]) ? $val[1] : $val[0]];
                    break;
                ...
                case 'border-bottom-left-radius':
                    ...
                    $this->value['border']['radius']['bl'] = [$val[0], isset($val[1]) ? $val[1] : $val[0]];
                    break;
        ...
        if ($noWidth && in_array($tagName,
                ['div', 'blockquote', 'fieldset']) && $this->value['position'] != 'absolute'
        ) {
            $this->value['width'] = $this->getLastWidth();
            $this->value['width'] -= $this->value['margin']['l'] + $this->value['margin']['r'];
        } else {
            if ($correctWidth) {
                if (!in_array($tagName, ['table', 'div', 'blockquote', 'fieldset', 'hr'])) {
