...
protected $_encoding = '';
                ...
                if ($res) {
                    ...
                    if (!in_array($res['name'], $tagsNotClosed)) {
                        ...
                        if ($res['close']) {
                            // HTML validation
                            if (count($parents) < 1) {
                                throw new HTML2PDF_exception(3, $res['name'],
                                    $this->getHtmlErrorCode($res['html_pos']));
                            } else {
                                if ($parents[count($parents) - 1] != $res['name']) {
                                    throw new HTML2PDF_exception(4, $parents,
                                        $this->getHtmlErrorCode($res['html_pos']));
                                } else {
            ...
            if ($part[0] == 'txt') {
                ...
                if (!$tagPreIn) {
                    ...
                    $actions[] = [
                        'name'  => 'write',
                        'close' => false,
                        'param' => ['txt' => $this->_prepareTxt($part[1])],
                    ];
                    ...
                    foreach ($part[1] as $k => $txt) {
                        ...
                        $actions[] = [
                            'name'  => 'write',
                            'close' => false,
                            'param' => ['txt' => $this->_prepareTxt($txt, false)],
                        ];
                    }
                }
            }
    ...
    protected function _searchCode(&$tmp)
    {
        ...
        while (preg_match($reg, $this->_html, $parse, PREG_OFFSET_CAPTURE, $offset)) {
            ...
            if ($parse[1][0]) {
                ...
                $tmp[] = ['code', trim($parse[1][0]), $offset];
    ...
    protected function _analiseCode($code)
    {
        ...
        if (in_array($name, ['ul', 'ol', 'table']) && !$close) {
            $this->_num++;
        ...
        return ['name' => $name, 'close' => $close ? 1 : 0, 'autoclose' => $autoclose, 'param' => $param];
    }
